...
class AddressComponent extends BaseComponent {
	...
	async getDistrictList(){
		return new Promise(async (resolve, reject) => {
	 		try{
	 			let result, url = 'http://apis.map.qq.com/ws/district/v1/list';
	...
	//根据adcode获取县区
	async getDistrictChildren(codeId){
		return new Promise(async (resolve, reject) => {
			try{
				let result, url = 'http://apis.map.qq.com/ws/district/v1/getchildren';
				result = await this.fetch(url,{id: codeId, key: this.tencentkey})
				if(result.status !== 0) {
					result = await this.fetch(url,{id: codeId, key: this.tencentkey2})
				}
				if(result.status == 0) {
					resolve(result.result)
	 			}else{
	 				console.log('获取行政区县失败', result)
	 				reject('获取行政区县失败')
	 			}
			}catch(err){
				reject(err);
